...
const Index = () => {
  const [loading, SetLoading] = useState(true),
    ...
    [day_msm, SetDayMsm] = useState(
      "Aún trabajo en esto, no me sale lo que quiero :v jaja"
    ),
    router = useRouter();
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (hora >= 22 && hora <= 6) {
        SetWelcome("Ya duerme Amor, ya es de noche :3");
        SetLoading(false);
      } else if (hora >= 6 && hora < 12) {
        SetWelcome("Buenos días Amor ^^");
        SetLoading(false);
      } else if (hora >= 12 && hora < 19) {
        SetWelcome("Buenas tardes Amorcito :3");
        SetLoading(false);
      } else if (hora >= 19 && hora <= 22) {
        SetWelcome("Buenas noches Princesa <3");
        SetLoading(false);
      }
    }
  }, []);
