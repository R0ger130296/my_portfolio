...
const Login = () => {
  const [user_email, setUser_email] = useState(""),
    [user_pass, setUser_pass] = useState(""),
    [allUsers, setAllUsers] = useState([]),
    [hist_sec, setHist_sec] = useState(),
    router = useRouter();
  ...
  useEffect(() => {
    ...
    db.ref("login-history").once("value", (element) => {
      ...
      hist_sec.sort(fromLargestToSmallest);
      let id = hist_sec;

      if (id.length === 0) {
        setHist_sec(1);
      } else {
        setHist_sec(parseInt(id) + 1);
      }
    });
  }, []);
  ...
  const loginAccess = (e) => {
    ...
    if (user_email === "" || user_pass === "") {
      ...
      if (userData) {
        if (userData.rol === "freetimeideas") {
          if (userData.password === user_pass) {
            db.ref("login-history/history" + hist_sec)
              .set({
                hist_sec: hist_sec,
                hist_user: user_email,
